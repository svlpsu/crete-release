...
#if !defined(AREG0)
//CPUState *env; TODO [CRETE]: uncertain if commenting this was the right move. Compiler complained about conflicting definitions.
#endif

/* Targets which don't use GETPC also don't need tci_tb_ptr
   which makes them a little faster. */
#if defined(GETPC)
//void *tci_tb_ptr; TODO [CRETE]: uncertain if commenting this was the right move. Compiler complained about conflicting definitions.
#endif

#if defined(CRETE_DEP_ANALYSIS)
static tcg_target_ulong crete_t0 = 0;
#endif // defined(CRETE_DEP_ANALYSIS)
...
unsigned long crete_tcg_qemu_tb_exec(CPUState *cpustate, uint8_t *tb_ptr)
{
...
#if defined(CRETE_DEP_ANALYSIS)
            crete_tci_ld8u_i32(crete_t0, t1, t2);
#endif // defined(CRETE_DEP_ANALYSIS)
            break;
...
#if defined(CRETE_DEP_ANALYSIS)
            crete_tci_ld_i32(crete_t0, t1, t2);
#endif // defined(CRETE_DEP_ANALYSIS)
            break;
...
#if defined(CRETE_CONFIG)
            // Skipping the TODO() call is risky, as we don't know the effect;
            // however it appears necessary for 64 bit interpreted mode.
            // Newer versions of QEMU eliminate the call, but whether that's because
            // they resolved the reason the TODO() was called in the first place is unknown.
...
#if defined(CRETE_DEP_ANALYSIS)
#if TCG_TARGET_REG_BITS == 32
            crete_tci_qemu_ld64_32(t0, t1, taddr);
#else // !(TCG_TARGET_REG_BITS == 32)
            crete_tci_qemu_ld64(t0, taddr);
#endif // TCG_TARGET_REG_BITS == 32
#endif // defined(CRETE_DEP_ANALYSIS)
            break;
...
#endif
#if defined(CRETE_DEP_ANALYSIS)
            crete_tci_qemu_st8(crete_t0, taddr);
#endif // defined(CRETE_DEP_ANALYSIS)
            break;
...
#endif
#if defined(CRETE_DEP_ANALYSIS)
            crete_tci_qemu_st16(crete_t0, taddr);
#endif // defined(CRETE_DEP_ANALYSIS)
            break;
...
#endif
#if defined(CRETE_DEP_ANALYSIS)
            crete_tci_qemu_st32(crete_t0, taddr);
#endif // defined(CRETE_DEP_ANALYSIS)
            break;

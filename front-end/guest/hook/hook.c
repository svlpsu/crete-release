...
void crete_symfile_memcpy(void* dst, void* src, size_t size)
{
    char* d = (char*)dst;
    char* s = (char*)src;
    size_t i;
...
CRETE_FILE *freopen(const char * pathname, const char * mode,
       CRETE_FILE * stream)
{
    if(!crete_symfile_is_stream_concolic(stream))
    {
...
// TODO: according to cplusplus.com, getc and fgetc are equivalent (so I can redirect here); however, gets and fgets are NOT equivalent.
...
int getc(CRETE_FILE *stream)
{
    ...
    assert(0 && "CRETE: unimplemented file routine called: getc");
    return 0;
}
...
int getc_unlocked(CRETE_FILE *stream)
{
    ...
    assert(0 && "CRETE: unsupported threading file routine called: getc_unlocked");
    return 0;
}
...
ssize_t getdelim(char ** lineptr, size_t * n,
       int delimiter, CRETE_FILE * stream)
{
    if(!crete_symfile_is_stream_concolic(stream))
    {
       ...
       CRETE_FILE * stream)
{
    ...
    assert(0 && "CRETE: unimplemented file routine called: getline");
    return 0;
}

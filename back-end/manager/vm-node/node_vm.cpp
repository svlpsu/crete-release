...
auto VMNodeUI::run() -> void
{
    ...
    node_ = boost::movelib::make_unique<AtomicGuard<VMNode>>(node_options_,
                                                             fs::current_path());

    AsyncTask async_task{NodeDriver<VMNode>{node_options_.master.ip,
                                            node_options_.master.port,
                                            *node_}};

    while(true) // TODO: interactively check cmd line here?
    {
        if(async_task.is_exception_thrown())
        {
            std::cerr << "VMNodeUI rethrowing exception" << std::endl;
...
int main(int argc, char* argv[])
{
    ...
    {
        cerr << "[boost] Exception: " <<  boost::diagnostic_information(e) << endl;
        return -1;
    }

...
int kTest_toFile(KTest *bo, const char *path) {
  ...
  { // Special local scope required b/c goto use.
      ...
      for (i=0; i<bo->numObjects; i++) {
          ...
          fprintf(stderr, "\n==================================\n"
                  "bo->numObjects = %d\n"
                  "[obj_%d] obj.address = %p, obj.numObjects = %u\n",
                  bo->numObjects, i, (void *)obj.address, obj.numBytes);
          );

          bool found = false;
          const concolics_ty& temp_concolics = g_qemu_rt_Info->get_concolics();
          for(concolics_ty::const_iterator it = temp_concolics.begin();
              it != temp_concolics.end(); ++it) {

              uint64_t concolic_addr = (*it)->m_guest_addr;
        	  uint32_t concolic_size = (*it)->m_data_size;
        	  string concolic_name = (*it)->m_name;

        	  uint64_t concolic_addr_end = concolic_addr + concolic_size;
        	  uint64_t obj_addr_end = (obj.address + obj.numBytes);

        	  if( (concolic_addr >= obj.address) &&
        			  (concolic_addr_end <= obj_addr_end) ){
                  crete::TestCaseElement elem;
                  elem.name = std::vector<uint8_t>(concolic_name.begin(), concolic_name.end());
                  elem.name_size = concolic_name.size();
                  elem.data_size = concolic_size;
                  for(uint32_t temp_i = 0; temp_i < concolic_size; ++temp_i){
                      uint32_t temp_index = concolic_addr - obj.address + temp_i;
                	  elem.data.push_back(obj.bytes[temp_index]);
                  }
          ...
          assert(found && "no concolic memory found associated with KTestObject");
      }

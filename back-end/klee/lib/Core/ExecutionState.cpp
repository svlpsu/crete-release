...
void ExecutionState::addSymbolic(const MemoryObject *mo, const Array *array) { 
  ...
  map_concolics_ty map_concolics = g_qemu_rt_Info->get_mapConcolics();
  map_concolics_ty::iterator map_it = map_concolics.find(mo->name);
  if (map_it == map_concolics.end()) {
      std::cerr << "[CRETE ERROR] The concrete value of a concolics variable name:"
              << mo->name << " not found!" << std::endl;
      assert(false);
  }

  ConcolicVariable *cv = map_it->second;
  concolics.add(array, cv->m_concrete_value);
#endif
}
///

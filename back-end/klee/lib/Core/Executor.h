...
private:
  ...
  void crete_init_qemu_memory(ExecutionState &state, QemuRuntimeInfo *qrt);
  void crete_init_special_function_handler();

  StatePair crete_concolic_fork(ExecutionState &current, ref<Expr> condition);

  void crete_handle_memory_missing(ExecutionState &state,
          bool isWrite,
          ref<Expr> address,
          ref<Expr> value,
          unsigned bytes,
...
private:
  ...
  void crete_init_qemu_globals(ExecutionState &state);

  MemoryObject *crete_merge_overlapped_mos(ExecutionState &state,
  		  uint64_t mo_start_addr, uint64_t mo_size);
  void crete_divide_memoSyncTable(ExecutionState &state, uint64_t tb_index,
  ...
  static void handleQemuTbPrologue(klee::Executor* executor,
...
private:
  // Map of predefined global values (Borrowed from S2E)
  // TODO: xxx being used to reconstruct qemu globals manually
  // in klee. They should be included in bit-code.

...
#if defined(CRETE_CONFIG)
	crete_init_qemu_memory(state, g_qemu_rt_Info);
#endif
...
void Executor::bindLocal(KInstruction *target, ExecutionState &state,
  ...
  if(ConstantExpr *CE = dyn_cast<ConstantExpr>(value)){
	  if(CE->getWidth() <= 64) {
		  uint64_t ce_value = CE->getZExtValue();
		  std::cerr << std::hex << "0x"<< ce_value << std::dec << std::endl;
	  }
  } else {
	  value->dump();
  }
...
void Executor::executeMemoryOperation(ExecutionState &state,
  ...
  if (!isa<ConstantExpr>(address)){
      ...
      CRETE_DBG_MEMORY(cerr << "[executeMemoryOperation] symbolic address = " << endl;
      sym_address->dump();
      cerr << "\nSymbolic address is concretized at: 0x"
              << hex << cast<ConstantExpr>(address)->getZExtValue()
              << dec << "!" << endl;
      );
  }
#endif // CRETE_CONFIG
...
#if defined(CRETE_CONFIG)
        crete_handle_memory_missing(state, isWrite, address,
                value, bytes, target);
        return;
#endif
...
/* Reconstruct the required memory from QEMU in klee's memory model
 * 1. Globals from qemu is reconstructed as external varial in klee. (To be optimized)
 * 2. Symbolic variables are constructed and initialized by 0s; they will be made as symbolic when
 * "__crete_make_symbolic" is invoked.
 *
 * TODO: xxx A better initialization flow for crete is:
 * 1. Reconstruct the initial hardware states from QEMU, like CPU state, device state
 * 2. Initialize symbolic variables
 * 3. (Globals from qemu should be included in bit-code, maybe all initial hardware state?)
 * */
void Executor::crete_init_qemu_memory(ExecutionState &state, QemuRuntimeInfo *qrt) {
	crete_init_symbolics(state);
	crete_init_qemu_globals(state);
}
...
void Executor::crete_init_special_function_handler() {
	  ...
	  function = kmodule->module->getFunction("qemu_tb_prelogue");
	  if(function)
	  {
		  specialFunctionHandler->addUHandler(function, handleQemuTbPrologue);
	  ...
	  function = kmodule->module->getFunction("raise_interrupt");
	  if(function)
	  {
		  specialFunctionHandler->addUHandler(function, handleQemuRaiseInterrupt);
		  if (!function->isDeclaration())
			  function->deleteBody();
	  }
#endif
...
/* Handle the missing MO issue:
 * 1. For write/store memory operation, just allocate the required MO/OS (if there are overlapped MOs,
 *    merge them to create a new big MO), and then redo the memory operation
 * 2. For read/load memory operation, we should never get it, so we assume it is just a result of page fault.
 */
void Executor::crete_handle_memory_missing(ExecutionState &state,
		bool isWrite,
		ref<Expr> address,
		ref<Expr> value,
		unsigned bytes,
    ...
    if(!isWrite) {
    	// Check whether the load mo missing is a result of page fault in QEMU
    	...
    	// If it is a result of page fault in QEMU, just abort the current_tb execution
    	if(qemu_interrupt_info.m_intno == EXCP0E_PAGE) {
    	    crete_abortCurrentTBExecution(&state);
    	    return;
    	} else {
    	    CRETE_DBG(
    ...
    if(is_overlapped){
    	crete_merge_overlapped_mos(state, temp_addr, bytes);
    } else {
    	// No overlapped existing mo is found, just create a new mo for this address and size
    	MemoryObject *temp_mo = memory->allocateFixed(temp_addr, bytes, 0);
...
void Executor::crete_init_symbolics(ExecutionState &state) {
	...
	for(concolics_ty::iterator it = temp_concolics.begin();
		...
		temp_mo = memory->allocateFixed((*it)->m_guest_addr, (*it)->m_data_size, 0);
...
void Executor::crete_init_qemu_globals(ExecutionState &state) {
#define __DEFINE_EXT_OBJECT_RO(name)                                    \
		predefinedSymbols.insert(std::make_pair(#name, (void*) &name)); \
		addExternalObject(*t_state, (void*) &name, sizeof(name),        \
				true)->setName(#name);
...
#define __DEFINE_EXT_OBJECT_RO_SYMB(name)                               \
	...
	ExecutionState* t_state = &state;

#if !defined(CRETE_QEMU10)
	__DEFINE_EXT_OBJECT_RO(qemu_loglevel)
	__DEFINE_EXT_OBJECT_RO(qemu_logfile)
#else
...
MemoryObject *Executor::crete_merge_overlapped_mos(ExecutionState &state,
		uint64_t mo_start_addr, uint64_t mo_size){
...
/* Check whether the entry in syncTable has a side effect:
 * Entries that have no side effect from memoSyncTable will be moved to a new
 * table noSideEffect_memoSyncTable.
 * */
void Executor::crete_divide_memoSyncTable(ExecutionState &state, uint64_t tb_index,
        memoSyncTable_ty& noSideEffect_syncTable)
{
	memoSyncTable_ty* memo_sync_table = g_qemu_rt_Info->get_memoSyncTable(tb_index);

	if(memo_sync_table->empty()){
		assert(noSideEffect_syncTable.empty());
		return;
	}

	// Contain the address of the entries that do not have side effect which will be removed from memo_sync_table
	vector<uint64_t> remove_entry_list;

	// Check each entry in the syncTable to see whether it has an side effect
	// The address of the ones that do not have side effect will be pushed to remove_entry_list
	for(memoSyncTable_ty::iterator it = memo_sync_table->begin();
			it != memo_sync_table->end(); ++it ) {
		uint64_t addr = it->second.m_addr;
	    uint32_t size = it->second.m_size;
	    std::vector<uint8_t> data = it->second.m_data;
	    assert(addr == it->first);

	    bool is_overlapped = memory->isOverlappedMO(addr, size);

	    // If there is no overlapped MO, current entry is totally new MO which for sure is a result of side effect
	    // As a result, only check side effect if there are overlapped MOs
	    if(is_overlapped){
	    	const MemoryObject *mo;
    		const ObjectState *os;

    		// Merge the overlapped MOs with given MO
	    	mo = crete_merge_overlapped_mos(state, addr, size);
	    	os = state.addressSpace.findObject(mo);
    		assert(mo && os && "mo or os is void pointer.\n");
    		assert( (mo->address <= addr) && (mo->address + mo->size >= addr + size) &&
    				"The given MO does not completely belong to the found MO.\n");

	    	// Get the current value, which will be used to do side effect check
    		unsigned offset = (unsigned)(addr - mo->address);
            ref<Expr> ref_current_value_byte;
            uint8_t current_value_byte;

            vector<uint8_t> v_current_value;
			for(unsigned int i = 0; i < size; ++i) {
				ref_current_value_byte = os->read8(offset + i);

				if(!isa<ConstantExpr>(ref_current_value_byte)) {
				    ref<Expr> sym_expr = state.constraints.simplifyExpr(ref_current_value_byte);
				    ref_current_value_byte = state.concolics.evaluate(sym_expr);
				}

				current_value_byte = (uint8_t)cast<ConstantExpr>(ref_current_value_byte)->getZExtValue(8);
				v_current_value.push_back(current_value_byte);
			}

            assert(v_current_value.size() == data.size());

            // Side effect check
            bool side_effect = false;
            for(uint32_t i = 0; i < size; ++i) {
            	if(data[i] != v_current_value[i]){
            		side_effect = true;
            		break;
            	}
            }
            ...
            if(!side_effect){
	            // if there is no side_effect, we do not need the current entry in memoSyncTable anymore
            	// and hence this entry will be removed (and moved to noSideEffect_memoSyncTable)
            	remove_entry_list.push_back(addr);
            }
	    }// if(!is_overlapped)
	}

	// Update memoSyncTable and noSideEffectTable based on remove_entry_list
	// Add the entries that does not have side-effect to the noSideEffect_syncTable,
	memoSyncTable_ty::iterator it_syncTb_entry;
	for(vector<uint64_t>::iterator it = remove_entry_list.begin();
	        it != remove_entry_list.end(); ++it){
	    it_syncTb_entry = memo_sync_table->find(*it);
	    noSideEffect_syncTable.insert(*it_syncTb_entry);
	}

	// Remove the entries that does not have side-effect from the memo_sync_table
	for(vector<uint64_t>::iterator it = remove_entry_list.begin();
	        it != remove_entry_list.end(); ++it){
	    memo_sync_table->erase(*it);
	}

	CRETE_DBG_MEMORY(cerr << "Output memoSyncTable: \n";
    g_qemu_rt_Info->printMemoSyncTable(tb_index);
    cerr << "Output noSideEffect_memoSyncTable: \n";
    g_qemu_rt_Info->printNoSideEffectMemoSyncTable(tb_index);
    cerr << "[Sync Table] crete_divide_memoSyncTable is done with tb_index = " << dec << tb_index << endl;
	);
}

/* Synchronize the memory between offline (klee) and online (qemu) by reconstruction and merging
 * Use memoSyncTable and noSideEffect_memoSyncTable dumped from QEMU to reconstruct/update the memory
 *  required for the execution of the coming TBs.
 *  1. noSideEffect_memoSyncTable will only be used to reconstruct memory(essentially merge MOs),
 *  2. memoSyncTable will also be used to update (concretize) related the value besides reconstruct memory
 * */
void Executor::crete_sync_memory(ExecutionState &state, uint64_t tb_index)
{
	memoSyncTable_ty noSideEffect_syncTable;
	memoSyncTable_ty* memo_sync_table;

    crete_divide_memoSyncTable(state, tb_index, noSideEffect_syncTable);

	// 1. noSideEffect_memoSyncTable will only be used to reconstruct memory(essentially merge MOs),
    uint64_t addr;
    uint32_t size;
	for(memoSyncTable_ty::iterator it = noSideEffect_syncTable.begin();
			it != noSideEffect_syncTable.end(); ++it ) {
		addr = it->second.m_addr;
		size = it->second.m_size;

		crete_merge_overlapped_mos(state, addr, size);
	}

	// 2. memoSyncTable will also be used to update (concretize) related the value besides reconstruct memory
	memo_sync_table = g_qemu_rt_Info->get_memoSyncTable(tb_index);
	for(memoSyncTable_ty::iterator it = memo_sync_table->begin();
			it != memo_sync_table->end(); ++it ) {
		addr = it->second.m_addr;
	    size = it->second.m_size;
	    ...
	    if(is_overlapped){
	    	...
	    	// Reconstruct memory by merging the overlapped MOs with given MO
	    	mo = crete_merge_overlapped_mos(state, addr, size);
	    	os = state.addressSpace.findObject(mo);
			assert(mo && os && "mo or os is void pointer.\n");
			assert( (mo->address <= addr) && (mo->address + mo->size >= addr + size) &&
			...
			// Update the related values based on the dumped value
			unsigned offset = (unsigned)(addr - mo->address);
	        ObjectState *wos = state.addressSpace.getWriteable(mo, os);
	        wos->write_n(offset, data);
	    } else {
	    	// if the given memo is not overlapped with any existed MO,
	    	// allocate a new MO and assign the corresponding dumped concrete value
	    	ObjectState  *temp_os;
	    	MemoryObject *temp_mo;

			temp_mo = memory->allocateFixed(addr, size, 0);
			temp_os = bindObjectInState(state, temp_mo, false);
			temp_os->write_n(0, data);
	    }
	}
}
...
{
    //TODO:xxx Virtual CPU state in qemu is reconstructed as a global variable
    // in klee named as tcg_env.
    MemoryObject *mo = crete_get_global("tcg_env");
    assert(mo);
    const ObjectState* os = state.addressSpace.findObject(mo);
    assert(os);

    ObjectState* wos = state.addressSpace.getWriteable(mo, os);
    g_qemu_rt_Info->update_regs(state, wos, tb_index);
}

MemoryObject* Executor::crete_get_global(string global_variable_name) const
{
    Module *m = kmodule->module;
    MemoryObject *mo = 0;
    string alloc_name;
        ...
        m->global_end(); i != e; ++i)
    {
      if (i->hasInitializer())
      {
        ...
        mo->getGlobalAllocName(alloc_name);

        if (alloc_name.compare(global_variable_name) == 0)
        {
            break;
        }
      }
    ...
    CRETE_DBG
    (cerr << "[crete_get_global] " << global_variable_name
            << ": address = 0x" << hex << mo->address
            << ", size = 0x" << mo->size << dec << endl;
    );
 ...
 * */
void Executor::handleQemuTbPrologue(klee::Executor* executor,
...
void Executor::handleCreteMakeSymbolic(klee::Executor* executor,
    ...
    ObjectPair res;
    assert(state->addressSpace.resolveOne(ConstantExpr::alloc(addr, Expr::Int64), res)
            && "crete_make_symbolic is failed\n");
